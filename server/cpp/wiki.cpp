...
int main(int argc, char** argv) {
	...
	if (action=="extract") {
		...
		Data d;
		cout<<"opened data, loading"<<endl;
		InMemoryData mem = d.to_mem();
		cout<<"loaded data"<<endl;

		gtl::vector<int> lowlink(d.n, -1);
		gtl::vector<int> active;
		gtl::vector<int> comp(d.n,-1);
		int ncomp=0;
		...
		vector<vector<int>> adj(d.n);
		for (int i=0; i<d.n; i++) {
			auto [f,t] = mem.adj(i, false);
			adj[i].assign(f,t);
		}
	
		// tarjan t; t.solve(edges, d.n);
		int kactl_comp_i=0;
		vector<int> comp2(d.n);
		kactl::scc(adj, [&](vector<int>& comp) {
			for (int x: comp) comp2[x]=kactl_comp_i;
			kactl_comp_i++;
		});
		cout<<"tarjan done with "<<kactl_comp_i<<" components, checking"<<endl;
		
		vector<int> comp_map(ncomp, -1);
		for (int i=0; i<d.n; i++) {
			if (comp_map[comp[i]]==-1) comp_map[comp[i]]=comp2[i];
			else if (comp_map[comp[i]]!=comp2[i]) {
				cout<<"tarjan: "<<comp2[i]<<" vs "<<comp_map[comp[i]]<<endl;
				throw runtime_error("mismatch");
			}
		}
	
		cout<<"done 1 way"<<endl;

		comp_map.assign(kactl_comp_i, -1);
		for (int i=0; i<d.n; i++) {
			if (comp_map[comp2[i]]==-1) comp_map[comp2[i]]=comp[i];
			else if (comp_map[comp2[i]]!=comp[i]) {
				cout<<"tarjan: "<<comp2[i]<<" vs "<<comp_map[comp[i]]<<endl;
				throw runtime_error("mismatch");
			}
		}
	
		// gtl::vector<gtl::vector<atomic<int>>> dist_to(d.n);
		// gtl::vector<gtl::vector<atomic<int>>> dist_from(d.n);

		// for (int j=0; j<d.n; j++) {
		// 	auto [f,t] = mem.adj(j, false);
		// 	dist_to[j].resize(t-f);
		// 	auto [f2,t2] = mem.adj(j, true);
		// 	dist_from[j].resize(t2-f2);
		// }

		// i=0;

		// gtl::vector<int> a2_vec(a2_cpy.begin(), a2_cpy.end());
		...
		// struct Out {
		// 	int max_d=0, p1=-1, p2=-1;
		// 	InMemoryData const& mem;
		// 	void pr() {
		// 		cout<<"max distance "<<max_d<<" from "<<mem.to_id[p1]<<" to "<<mem.to_id[p2]<<endl;
		// 	}
		// };

		// vector<Out> outs(nt, {.mem=mem});

		// pool.launch_all([&a2_i, &a2_vec, n=d.n, &dist_from, &dist_to, &mem, &outs](int ti) {
		// 	gtl::vector<int> a,b,all;
		// 	gtl::vector<bool> vis(n);
			
		// 	auto& out = outs[ti];

		// 	while (true) {
		// 		int j = a2_i.fetch_add(1);
		// 		if (j>=a2_vec.size()) break;
		// 		int x = a2_vec[j];
		...
		// 					auto& dv = rev ? dist_from[v] : dist_to[v];
		// 					auto [f,t] = mem.adj(v, rev);
		...
		// 				if (b.size() && dist>out.max_d) {
		// 					out.max_d=dist;
		// 					out.p1=x, out.p2=b[0];
		// 					if (rev) swap(out.p1, out.p2);

		// 					cout<<"thread "<<ti<<" found new best: ";
		// 					out.pr();
		// 				}
		...
		// 		cout<<"node "<<++j<<"/"<<a2_vec.size()<<", "<<k<<" visited"<<endl;
		// 	}
		// }, nt);
		...
		// Out* out=nullptr;
		// for (int j=0; j<nt; j++) {
		// 	if (!out || outs[j].max_d > out->max_d) out=&outs[j];
		// }
